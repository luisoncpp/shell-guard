// @Architecture(type=Module, descriptionShort="The verb contract every shgd verb implements", descriptionLong="Verbs return lines and an exit code instead of printing, so index.ts can apply output shaping and shgd batch can label and concatenate several verbs in one invocation. A verb that calls console.log is invisible to both. Declared in its own module so verbs and batch never import index.ts, which would be circular.")
