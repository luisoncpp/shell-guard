// @Architecture(type=Module, descriptionShort="Pure parsing and admission rules for shgd batch steps", descriptionLong="Turns each quoted step string into an argv array and admits it only if the verb exists, is not batch itself, and carries no write flag. This is the boundary that keeps batch from becoming a shell: a step names a verb from shgd's own table, never a program, a script or a pipeline.")
...
/** `--take` is shgd's only write path; a destructive step must stay a visible, standalone call. */
...
export function planSteps(rawSteps: readonly string[], knownVerbs: ReadonlySet<string>): BatchStep[] {
  if (rawSteps.length === 0) throw new Error('usage: shgd batch "<verb args>" "<verb args>" ...');
