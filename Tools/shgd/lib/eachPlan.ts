// @Architecture(type=Module, descriptionShort="Pure per-file summarisation rules for shgd each", descriptionLong="Decides what one file contributes to the report given the requested mode, so the loop an agent would otherwise write in shell (for f in ...; do head -1 $f; done) lives inside a verb and is unit-testable. Takes file contents as a string; the verb does the reading.")
 ...
 * shaping option, and a mode sharing its name would silently steal it from every `shgd each`.
 */
export function resolveEachRequest(flags: ReadonlySet<string>, options: ReadonlyMap<string, string>): EachRequest {
  ...
  if (requested.length === 0) {
    throw new Error('shgd each needs a mode: --cat | --first=N | --first-line | --count-lines | --count=<regex>');
  }
