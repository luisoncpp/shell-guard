// @Architecture(type=Module, descriptionShort="Single choke point for every shgd file write", descriptionLong="Enforces the write toggle, repository containment (through symlinks, via repoFile) and a protected-path list before writing, and copies the current file to SHGD_TMP/backups first. The protected list re-implements Claude Code's own, because a blanket Bash(shgd:*) allow rule bypasses that permission layer; any verb writing with bare writeFileSync silently defeats this guard.")
...
export function isWriteEnabled(): boolean {
  return process.env.SHGD_WRITE !== '0' && process.env.SHGD_NO_WRITE !== '1';
}
...
export function assertWritable(target: string): string {
  if (!isWriteEnabled()) {
    throw new Error('shgd writes are disabled (SHGD_WRITE=0 / SHGD_NO_WRITE=1). Re-run without it to allow edits.');
  }
