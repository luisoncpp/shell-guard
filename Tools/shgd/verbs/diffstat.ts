...
import { DefaultDiffBase, Limits, TestPathFragment } from '../lib/constants';
...
export function diffstat(args: ParsedArgs): VerbResult {
  const base = assertSafeGitArgument(args.positional[0] ?? DefaultDiffBase, 'base');
  ...
  const substantive = countSubstantiveLines(runGit(['diff', '-U0', range]).stdout);
