// @Architecture(type=Module, descriptionShort="Commit provenance for a file, a branch or a vanished path", descriptionLong="Three questions that each used to need a shell loop or a command pair: what last touched this file and is it dirty (--file), which commit changed what across a branch (--commits), and where did a path that no longer exists live (--find, searching all refs). `shgd show <ref>:<path>` prints a file as of a revision.")
...
export function show(args: ParsedArgs): VerbResult {
  ...
  if (!spec) throw new Error('usage: shgd show <ref>:<path>');
