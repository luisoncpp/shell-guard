// @Architecture(type=Module, descriptionShort="tl CLI entry: argv parse, verb dispatch, output shaping", descriptionLong="Parses argv into flags/options/positionals/pathspecs, dispatches to one verb from a frozen table, then applies head/tail/grep/max-cols shaping to the lines the verb returned. Verbs never print; this is the only place output reaches stdout, which is what lets batch relabel and concatenate them. Thrown errors map to exit 1, an unknown verb to exit 2.")
...
function main(): number {
  // `npm run tl -- where` has npm eat the separator, the PATH shim does not, so the
  ...
  if (args.flags.has('no-write')) process.env.TL_NO_WRITE = '1';
...
try {
  ...
  console.error(`tl: ${(error as Error).message}`);
  process.exit(1);
}
