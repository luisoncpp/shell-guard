// @Architecture(type=Module, descriptionShort="Pure argv tokenisation for tl", descriptionLong="Splits an argument list into bare --flags, --key=value options, positionals and the pathspecs that follow a literal --. Values are accepted only in the --key=value form so that a verb taking positionals (tl fallow audit origin/main) can never confuse an option's value for one of its own arguments. A single-dash token is rejected at parse time rather than treated as a positional, because no verb has a short option and a token like -n would otherwise become a verb's first argument in silence.")
...
export function parseArgs(argv: readonly string[]): ParsedArgs {
  ...
  for (const argument of argv) {
    ...
    if (!argument.startsWith(OptionPrefix)) {
      // A single-dash token is never a tl option, and letting it fall through to
      // positional is worse than useless: assertKnownKeys only inspects flags and
      // options, so `tl grep -n <pattern>` would take -n as the pattern and demote
      // the real pattern to a pathspec — a silently wrong search, not an error.
      if (argument.startsWith(ShortOptionPrefix)) {
        throw new Error(`unknown flag "${argument}"; tl options take the --flag or --key=value form`);
      }
