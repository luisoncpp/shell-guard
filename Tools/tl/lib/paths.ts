// @Architecture(type=Module, descriptionShort="Repo root, tmp and backup directory resolution", descriptionLong="Resolves the repository root from git rather than the process cwd (which drifts mid-session), plus the tmp directory used for temporary files and write pre-images, honouring TL_TMP. Also holds the repo-containment predicate the write guard depends on — which resolves symlinks and Windows junctions first, because a lexical check is not containment.")
