...
export function resolveExecutable(command: string): string {
  ...
  throw new Error(`${command} was not found on PATH; tl will not run it by bare name because the current directory would be searched first`);
}
...
export function runTool(command: 'npm' | 'npx', args: readonly string[]): CapturedRun {
  ...
  if (!isWindows()) return spawnCaptured(executable, args, /*useShell=*/false);
  // The resolved path routinely contains a space (`C:\Program Files\nodejs\npm.cmd`)
  // and Node will not quote it either.
  return spawnCaptured(`"${executable}"`, args, /*useShell=*/true);
}
