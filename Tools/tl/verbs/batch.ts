// @Architecture(type=Module, descriptionShort="Runs several tl verbs in one invocation", descriptionLong="Collapses N tool calls into one round-trip with labelled, independently-shaped output. It once existed for permissions — a semicolon joining two allowed commands used to prompt — but Claude Code now allows a compound line when every part matches a rule, so that argument has expired and the round-trip saving is what remains. Steps are dispatched through tl's own verb table (injected to avoid importing index.ts), labelled, and concatenated. Steps are sequential and independent — there is deliberately no data flow between them, because a substitution syntax would make batch a programming language.")
...
function runStep(step: BatchStep, dispatch: StepDispatch): StepOutcome {
  try {
    ...
    return { lines: [`tl: ${(error as Error).message}`], code: 1 };
  }
}
